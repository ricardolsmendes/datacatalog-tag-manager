...
class TagDatasourceProcessor:
    ...
    def __create_tags_from_dataframe(self, dataframe):
        ...
        for linked_resource in normalized_df.index.unique().tolist():
            ...
            templates_subset = normalized_df.loc[[linked_resource], constant.TAGS_DS_TEMPLATE_NAME_COLUMN_LABEL:]
    ...
    def __normalize_dataframe(cls, dataframe):
        ...
        rebuilt_df = pd.concat([filled_subset, ordered_df[constant.TAGS_DS_NON_FILLABLE_COLUMNS]], axis=1)
    ...
    def __create_tags_from_templates_dataframe(self, dataframe):
        ...
        for template_name in dataframe.index.unique().tolist():
            ...
            columns_subset = dataframe.loc[[template_name], constant.TAGS_DS_SCHEMA_COLUMN_COLUMN_LABEL:]
            ...
            null_columns_index = columns_subset[constant.TAGS_DS_SCHEMA_COLUMN_COLUMN_LABEL].isnull()
            null_columns_subset = columns_subset.loc[null_columns_index, constant.TAGS_DS_FIELD_ID_COLUMN_LABEL:]

            if not null_columns_subset.empty:
                columns_subset.dropna(subset=[constant.TAGS_DS_SCHEMA_COLUMN_COLUMN_LABEL], inplace=True)
